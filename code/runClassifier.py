...
def quantizeY(org_y, quantize_levels):
    new_org_y = []
    for y in org_y:
        left_over = y % quantize_levels
        y -= left_over
        new_org_y.append(y)
    new_org_y = reshape(new_org_y, (-1, 1))
    return new_org_y
...
def computeClassificationAcc(org_y, predicted_y):
    ...
    '''
    accuracy = np.mean((org_y - np.sqrt((org_y-predicted_y)**2))/org_y)
    return accuracy

def computeAvgRegrMSError(org_y, predicted_y):
    '''
        Compute regression error by average error between predicted_y
        and org_y. Use L2 distance between two values (each element
        in the vector).
    '''
    error = np.sqrt(np.mean((org_y - predicted_y)**2))
    return error
